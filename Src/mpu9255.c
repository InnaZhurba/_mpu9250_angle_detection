...
// history of roll!!!]
float roll_history[2]={0,0};

// history of pitch!!!]
float pitch_history[2]={0,0};

// history of pitch!!!]
float yaw_history[2]={0,0};
...
/* Find coordinates of the initial position of gyroscope */
void test_values(I2C_HandleTypeDef *I2Cx, MPU9255_t*DataStruct)
{
...
void readAll(I2C_HandleTypeDef *I2Cx, MPU9255_t*DataStruct) {
	...
	if (Data & 0x01) {  // On interrupt, check if data ready interrupt
		...
		ax = ((float)accelCount[0]-accelBias[0])*aRes; //- Axoff;// - accelBias[0];  // get actual g value, this depends on scale being set
		ay = ((float)accelCount[1]-accelBias[1])*aRes; //- Ayoff; // - accelBias[1];
		az = ((float)accelCount[2]-accelBias[2])*aRes; //- Azoff;// - accelBias[2];
		...
		gx = (float)gyroCount[0]*gRes; //- Gxoff;// - gyroBias[0];  // get actual gyro value, this depends on scale being set
		gy = (float)gyroCount[1]*gRes; //- Gyoff;// - gyroBias[1];
		gz = (float)gyroCount[2]*gRes; //- Gzoff;// - gyroBias[2];
	...
	pitch *= 180.0f / PI;
	yaw   *= 180.0f / PI;
	yaw   += 7.53f; // Declination

	if(yaw < 0) yaw   += 360.0f; // Ensure yaw stays between 0 and 360
	roll  *= 180.0f / PI;*/

	  //Комплементарний фільтр
	//int new_ax = ax*0.4 + gx *(1-0.4);
	//int new_ay = ay*0.4 + gy *(1-0.4);
	...
	float angle_Ax=0,angle_Ay=0,angle_Az=0;
	      ...
	      Ax = (accelCount[0] - Axoff)/16384.0;
		  Ay = (accelCount[0] - Ayoff)/16384.0;
		  Az = (accelCount[0]-(Azoff-16384))/16384.0;

		  Gx = (gyroCount[0] - Gxoff)/131.0;
		  Gy = (gyroCount[1] - Gyoff)/131.0;
		  Gz = (gyroCount[2] - Gzoff)/131.0;

		  //converting to angles
		  	  float FK = 0.9;
		  	  //DataStruct->my_roll  = atan2f(Ay, Az) * 180/M_PI;
		  	  //DataStruct->my_pitch = atan2f(Ax, sqrt(Ay*Ay + Az*Az)) * 180/M_PI;
		  	  ...
		  	  //Комплементарний фільтр
		  	DataStruct->my_roll = angle_Ax*FK + angle_Gx *(1-FK);
		  	DataStruct->my_pitch = angle_Ay*FK + angle_Gy *(1-FK);
	    ...
	    roll = (roll_history[0] + roll_history[1])/2; //(roll_history[0] + roll_history[1])/2

	    //update pitch history
	    pitch_history[1]= pitch_history[0];
	   	pitch_history[0] = pitch;

	    pitch = (pitch_history[0] + pitch_history[1])/2; //(roll_history[0] + roll_history[1])/2

	    //update yaw history
	    yaw_history[1] = yaw_history[0];
	    yaw_history[0] = yaw;

	    yaw =  (yaw_history[0] + yaw_history[1])/2;
